...
class Subject(models.Model):
    ...
    group_ib = models.ForeignKey('curriculum.IbSubjectGroup', verbose_name=_("Предметная группа в IB"), on_delete=models.SET_NULL, null=True, related_name="subjects")
    group_fgos = models.ForeignKey('curriculum.FgosSubjectGroup', verbose_name=_("Предметная область в РФ"), on_delete=models.SET_NULL, null=True, related_name="subjects")
...
class CurriculumLoad(models.Model):
    ...
    years = models.ManyToManyField('general.StudyYear', verbose_name=_("Параллель"), related_name="curriculum_loads")
    ...
    def __str__(self):
        return f"{self.curriculum} ({self.subject} - {self.hours})"
