...
<script>
export default {
  name: 'unit-field-checkbox-search',
  props: {
    fieldName: { String },
    fieldText: { Object },
  ...
  computed: {
    ...
    searchOptions() {
      if (!this.search) {
        return this.options.filter(item => this.editData[`${this.fieldName}_ids`].includes(item.id))
      }
      return this.options.filter((item) => {
        if (item[this.fieldSearch]) {
          return item[this.fieldSearch].toLowerCase().includes(this.search.toLowerCase()) || this.editData[`${this.fieldName}_ids`].includes(item.id)
        } else {
          return this.editData[`${this.fieldName}_ids`].includes(item.id)
        }
      })
    }
  },
