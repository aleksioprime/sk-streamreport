  ...
  <form>
    ...
    <div class="field-admin">
      <div class="form-check my-2">
        <input class="form-check-input" type="checkbox" :value="true" id="teacher-admin" v-model="editedUser.teacher.admin">
        <label class="form-check-label" for="teacher-admin">
          Администрация
        </label>
      </div>
    </div>
    <div class="row my-2">
      <div class="col">
        <label for="teacher-position" class="form-label">Должность:</label>
        <input id="teacher-position" class="form-control" type="text" v-model="editedUser.teacher.position">
        <small ref="position_alert" class="alert-text"></small>
      </div>
    </div>
    <div class="row my-2">
      <div class="col">
        <label for="id-dnevnik-teacher" class="form-label">ID системы Дневник:</label>
        <input id="id-dnevnik-teacher" class="form-control" type="text" v-model="editedUser.teacher.id_dnevnik">
...
export default {
  props: {
    ...
    validFormEmployee: { type: Boolean },
  },
  setup(props) {
    return {
    }
  ...
  methods: {
    ...
    checkFieldsValidate() {
      ...
      this.$emit('update:validFormEmployee', validate);
      this.validateForm();
    },
