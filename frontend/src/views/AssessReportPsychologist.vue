<template>
  <div>
    <base-header>
      <template v-slot:header>Репорты психологов</template>
    ...
    <div v-if="!isGroupLoading">
      <div v-for="(groupsByYear, year) in groupedArrayData(filteredGroups, ['class_year', 'year_rus'])" :key="year">
        ...
        <div v-for="group in groupsByYear" :key="group.id" class="class-item area">
            ...
            <div class="psychologist-period" v-if="group.psychologist">
              <div class="period-item" v-for="period in reportPeriods.filter(item => !currentReportPeriod.id || item.id == currentReportPeriod.id)" :key="period.id">
                ...
                <div class="period-info">Психолог: {{ group.psychologist.full_name }}</div>
                ...
                <button @click="$router.push(`/report/psychologist/group/${group.id}/period/${period.id}`)" class="btn btn-primary period-button mt-2">Перейти к репортам</button>
              </div>
            </div>
            <div v-else>Психолог не назначен</div>
        </div>
      </div>
    </div>
...
import { getGroupsForReportPsycho } from "@/hooks/user/useGroup";
...
export default {
  ...
  setup(props) {
    ...
    const { groupsForReportPsycho, isGroupLoading, fetchGetGroupsForReportPsycho } = getGroupsForReportPsycho();
  ...
  methods: {
    ...
    choiceYear() {
      localStorage.setItem('report_psycho_year', this.currentYearId);
      if (this.showAllData) {
        this.fetchGetGroupsForReportPsycho({ study_year: this.currentStudyYear.id, class_year: this.currentYearId });
      } else {
        this.fetchGetGroupsForReportPsycho({ study_year: this.currentStudyYear.id, class_year: this.currentYearId, psychologist: this.authUser.teacher.id });
      }
    },
    changeView() {
      this.currentYearId = null;
      localStorage.removeItem('report_psycho_year');
      this.getDataForReports();
      if (this.showAllData) {
        localStorage.setItem('report_psycho_all', true);
      } else {
        localStorage.removeItem('report_psycho_all');
      }
    },
    ...
    recoveryDataFromLocalStorage() {
      this.currentYearId = Number(localStorage.getItem('report_psycho_year')) || null;
      this.showAllData = Boolean(localStorage.getItem('report_psycho_all')) || false;
    },
    getDataForReports() {
      if (this.showAllData) {
        ...
        this.fetchGetReportPeriods({ study_year: this.currentStudyYear.id });
        this.fetchGetClassYears({ study_year: this.currentStudyYear.id, psychologist: this.authUser.teacher.id });
        this.fetchGetGroupsForReportPsycho({ study_year: this.currentStudyYear.id, class_year: this.currentYearId, psychologist: this.authUser.teacher.id });
      }
    }
  },
  ...
  computed: {
    filteredGroups() {
      return this.groupsForReportPsycho.filter(item => {
